...
<%@page import="com.itwill.shop.cs.CsListPageMakerDto"%>
<%@page import="com.itwill.shop.cs.PageInputDto"%>
<%@page import="com.itwill.shop.cs.util.PageMaker"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>


<%!public String getTitleString(Cs cs) {
    StringBuilder title = new StringBuilder(128);
    String t = cs.getCsTitle();
    if (t.length() > 30) {
        t = String.format("%s...", t.substring(0, 15));
    }
    for (int i = 0; i < cs.getCsDepth(); i++) {
        title.append("&nbsp;&nbsp;");
    }
    if (cs.getCsDepth() > 0) {
...
//게시물조회
CsListPageMakerDto csListPage = 
    new CsService().findCsList(Integer.parseInt(pageno));
     ...
     <script type="text/javascript">
     	function csCreate() {
    		 if (<%=!isLogin%>) {
					...
					location.href = "cs_write.jsp";
					return true;
				}
    	     }
   	  function checkLogin(csNo, pageno) {
			    if (<%=!isLogin%>) {
			        alert('로그인 하세요.');
			        location.href = 'user_login_form.jsp';
			        return false
			    } else {
			        location.href = 'cs_view.jsp?csNo=' + csNo + '&pageno=' + pageno;
			        return true;
			    }
			}
	</script>
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<table style="margin-left: auto; margin-right: 0;" border="0"
					...
					<tr bgcolor="#FFFFFF">
						<td height="20" class="t1" align="right" valign="bottom">♠ 총
							<font color="#FF0000"><%=csListPage.pageMaker.getTotCount()%></font>
							건 | 현재페이지(<font color="#FF0000"><%=csListPage.pageMaker.getCurPage()%></font>
							/ <font color="#0000FF"><%=csListPage.pageMaker.getTotPage()%></font>)
						</td>
					</tr>
				</table>
				<table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <%if(cs.getRn()<4){
                            	...
                            	out.print(cs.getRn()-3);
                            }
                            %>
                            </td>
	                        <td ><a href="cs_view.jsp?csNo=<%= cs.getCsNo() %>&pageno=<%=csListPage.pageMaker.getCurPage()%>"
									onclick="checkLogin(<%= cs.getCsNo() %>, <%= csListPage.pageMaker.getCurPage() %>); return false;">
	                                <%= this.getTitleString(cs) %></a></td>
	                        <td align="center"><%= cs.getUId() %></td>
	                        <td align="center"><%= dateFormat.format(cs.getCsDate()) %></td>
	                        <td align="center"><%= cs.getCsReadCount() %></td>
                        </tr>
                        <% } %>
                    </tbody>
                </table>
				...
				<table class="table" id="paging_table">
					<tr>
						<td align="center">
							<%if(csListPage.pageMaker.getCurBlock() > 1) {%> <a
							href="./cs_list.jsp?pageno=<%=csListPage.pageMaker.getPrevGroupStartPage()%>">◀◀</a>&nbsp;&nbsp;
							<%}%> <%if(csListPage.pageMaker.getPrevPage()>0) {%> <a
							href="./cs_list.jsp?pageno=<%=csListPage.pageMaker.getPrevPage()%>">◀</a>&nbsp;&nbsp;
							<%}%> <%
								for (int i = csListPage.pageMaker.getBlockBegin(); i <= csListPage.pageMaker.getBlockEnd(); i++) {
							 	if (csListPage.pageMaker.getCurPage() == i) {
							%> <font color='red'><strong><%=i%></strong></font>&nbsp; <%} else {%>
							<a href="./cs_list.jsp?pageno=<%=i%>"><strong><%=i%></strong></a>&nbsp;
							<%
							   }
							  }%> <%if(csListPage.pageMaker.getNextPage()<=csListPage.pageMaker.getTotPage()){%>
							<a
							href="./cs_list.jsp?pageno=<%=csListPage.pageMaker.getNextPage()%>">▶&nbsp;&nbsp;</a>
							<%}%> <%if(csListPage.pageMaker.getTotBlock() > csListPage.pageMaker.getCurBlock()){%>
							<a
							href="./cs_list.jsp?pageno=<%=csListPage.pageMaker.getNextGroupStartPage()%>">▶▶&nbsp;&nbsp;</a>
							<%}%>
						</td>
					</tr>
					<tr>
						<td align="right"><input type="button" value="글쓰기"
							class="genric-btn primary-border" onclick="csCreate();" /></td>
					</tr>
				</table>
            </div>
        </div>

...
<%
	...
	int pageno=1;
	try{
		csNo=Integer.parseInt(request.getParameter("csNo"));
	...
	if(csNo==null){
		//목록으로이동
		response.sendRedirect("cs_list.jsp?pageno="+pageno);
		return;
	}
	...
	//읽은회수증가
	new CsService().updateHitCount(csNo);
%>
...
<head>
	...
	<script language="JavaScript">
		function csCreate() {
			f.action = "cs_write.jsp";
			f.submit();
		}
		/*
		function csReplyCreate() {
			document.f.action = "cs_reply_write.jsp";
			document.f.method='POST';
			document.f.submit();
		}
		*/
		function csUpdate() {
			document.f.action = "cs_modify.jsp";
			document.f.submit();
		}
		function csRemove() {
			if (confirm("삭제하시겠습니까?")) {
				document.f.action = "cs_remove_action.jsp";
				document.f.submit();
			}
		}
		function csList() {
			f.action = "cs_list.jsp?pageno="+<%=pageno%>;
			f.submit();
		}
	</script>
</head>
	                ...
	                <table class="table">
                        ...
                        <tr>
                        	<td>
                        		...
                        		<input type="text" class="form-control" name="csTitle" maxlength="50" value="<%=cs.getCsTitle()%>" readonly="readonly">
                        	</td>
                        </tr>
                        <tr>
                            <td>
                            	<label for="f-option3">내용</label><br><br>
                            	<textarea class="form-control" name="csContent" maxlength="3000" style="height: 350px; resize:none" readonly="readonly"><%=cs.getCsContent()%></textarea>
                            </td>
                        </tr>
                        <tr>
						<td align=center>
							<input type="button" value="글쓰기" class="genric-btn primary-border" onClick="csCreate()"> &nbsp; 
							<input type="button" value="고객센터 리스트" class="genric-btn primary-border" onClick="csList()"> &nbsp;
						<%
                        // 로그인한 사용자와 게시물 작성자의 아이디가 일치하는지 확인
                        boolean isMatchUser = sUserId.equals(cs.getUId());
                        %>
                        <!-- 수정, 삭제 버튼은 로그인 사용자와 작성자가 동일한 경우에만 보이도록 함 -->
                        <% if (isMatchUser) { %>
                               <input type="button" value="수정" class="genric-btn primary-border" onClick="csUpdate()"> &nbsp;
                               <input type="button" value="삭제" class="genric-btn primary-border" onClick="csRemove()">
                        <% } %>	
						</td>
					</tr>
	                </table>

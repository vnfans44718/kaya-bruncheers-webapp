...
for (Order order : orderList) {
	String OrderPImg = orderService.findPrimaryImage(order.getONo());
	order.setOImg(OrderPImg);
}
%>
...
function oNo_send(index){
	document.getElementById('oNo_f_'+index).method='POST';
    document.getElementById('oNo_f_'+index).action='order_detail.jsp';
    document.getElementById('oNo_f_'+index).submit();
}
			...
			<div class="cart_inner">
				<div class="table-responsive">
					<table class="table">
						...
						%>
						<div>
						<td style="text-align: center;">주문내역이 존재하지 않습니다.</td>
						<td></td>
						<td></td>
						<td></td>
						</div>
							...
							%>
										<div class="d-flex">
											<img width="150" height="150" src="img/product/<%=order.getOImg() %>" alt="">
										</div>
										<div class="media-body">
										<!--  oNo 만 보내자.... -->
										<form id="oNo_f_<%=index %>" name="oNo_f_<%=index %>">
											<a href="javascript:oNo_send(<%=index %>);" style="color: #1C1C1C;" >
											<input type="hidden" name="oNo" id="oNo" value="<%=order.getONo()%>">
												주문번호:
												<%=order.getONo()%>
												</a>
											</form>
										<!--  oNo 만 보내자.... -->
											<p>
												 &#39;&nbsp;&nbsp;<%=order.getOImg().substring(0, order.getOImg().lastIndexOf('.'))%>&nbsp;&nbsp;&apos;
												외 <%=order.getOrderItemList().size() %>종
											</p>
										</div>
									</div>
								</td>
								<td>
									<h5><%=new SimpleDateFormat("yyyy/MM/dd").format(order.getODate())%></h5>
								</td>
								<td>
									<h5>
										<h5><%=new DecimalFormat("#,###").format(order.getOPrice())%>원
										</h5>
									</h5>
								</td>
								<td>
									<div class="product_count">
										<input type="hidden" name="qty" id="sst" maxlength="12"
											value=""
